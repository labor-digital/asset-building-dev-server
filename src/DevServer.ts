...
function rewriteAppConfig(context: WorkerContext): void
{
    ...
    if (typeof config.entry !== 'object' || !Array.isArray(config.entry.main)) {
        throw new Error(
...
export default function (context: CoreContext, scope: string) {
    if (scope !== 'global') {
        throw new Error('The dev server extension has to be registered on a global scale!');
    }
    if (context.builderVersion !== 2) {
        throw new Error('The dev server only works with builder version 2 definitions!');
    }
    ...
    context.eventEmitter.bind(AssetBuilderEventList.FILTER_WEBPACK_COMPILER, (e) => {
        ...
        if (context.app.disableDevServer) {
            return;
        }
